<script lang="ts">
  ...
  function sceneUrlFor(algo: AlgoVersion): string {
    const base = (import.meta as ImportMeta & { env: { BASE_URL: string } }).env.BASE_URL;
    return algo === "v2" ? `${base}data/museeka_demo_scene_v2.json` : `${base}data/museeka_demo_scene.json`;
  }
  ...
  async function switchAlgo(next: AlgoVersion) {
    ...
    // Tear down current runtime/renderer and load the other scene file.
    cancelAnimationFrame(frame);
    detachDriver();
    renderer?.dispose();
    runtime?.dispose();
  ...
  {#if scene && runtime && samplesReady}
    <section class="hud" class:collapsed={hudCollapsed} aria-label="Contrôles Museeka">
      ...
      <div class="mode-switch" aria-label="Algorithme">
        <button class:active={algo === "v1"} on:click={() => switchAlgo("v1")} data-testid="algo-v1" title="Algo V1 — filtre runtime par parcours">V1</button>
        <button class:active={algo === "v2"} on:click={() => switchAlgo("v2")} data-testid="algo-v2" title="Algo V2 — espace partagé, aucun filtre runtime">V2</button>
      </div>
